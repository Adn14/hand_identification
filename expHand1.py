# -*- coding: utf-8 -*-
"""
Created on Thu May 24 22:38:30 2018
...
# -*- coding: utf-8 -*-
"""
Created on Thu May 24 16:36:22 2018
...
def hsvskin(val, no, fold):
    ...
    scipy.misc.imsave('percobaan/'+ str(fold)+'/outfile'+str(no)+'.jpg', img)
    
    img = cv2.imread('percobaan/'+ str(fold)+'/outfile'+str(no)+'.jpg')
...
def findHand(val, no, fold, cutFold):
    no = no
    fold = fold
    image = cv2.imread("percobaan/"+fold+"/outfile"+str(no)+".jpg")
    ...
    #cv2.imshow("cut", cut )
    scipy.misc.imsave("C:/Users/aragu/.spyder-py3/percobaan/"+cutFold+"/outcut"+str(no)+".jpg", cut)
    return cut
...
def exp(val,no,fold):
    ...
    hasil_hsv = hsvskin(hsv(val), no, fold)
...
#seg1, seg2, seg3, seg4 = crop24(skel)
...
for j in range(0,5):
    file = ["satu","dua","tiga","empat","lima"]
    fold = ["outSatu", "outDua", "outTiga", "outEmpat","outLima"]
    cutFold = ["cutSatu", "cutDua", "cutTiga", "cutEmpat", "cutLima"]
    
    for i in range(1,11):
        arData = []
        im = cv2.imread("C:/Users/aragu/.spyder-py3/percobaan/"+file[j]+"/"+ str(i) +".jpg")
        exp(im, i, fold[j])
        tangan = findHand(exp, i, fold[j], cutFold[j])
        skelet = skelFocus("percobaan/"+cutFold[j]+"/outcut"+str(i)+".jpg")
        seg1, seg2, seg3, seg4 = crop24(skelet)
        print("seg 1 : ", np.sum(seg1, axis = None))
        print("seg 2 : ", np.sum(seg2, axis = None))
        print("seg 3 : ", np.sum(seg3, axis = None))
        print("seg 4 : ", np.sum(seg4, axis = None))
        ...
        myFile = open("csvTes4.csv", "a", newline='')
        with myFile:
